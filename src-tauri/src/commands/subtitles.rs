...
use crate::server::types::SubtitleSearchResult;
...
pub async fn search_subtitles(
    ...
    client.search_subtitles(&imdb_id, languages.as_deref()).await
}
...
pub async fn download_subtitle(
    ...
    println!("[download_subtitle] Downloading from server: {}", download_url);
    let client = client.read().await;

    // Fetch VTT from server's subtitle download endpoint
    let url = client.subtitle_download_url(&download_url);
    ...
    let vtt_bytes = resp.bytes().await
        .map_err(|e| format!("Failed to read subtitle data: {}", e))?;

    // Encode as base64 data URL for <track> element
    use base64::Engine;
    let base64_content = base64::engine::general_purpose::STANDARD.encode(&vtt_bytes);
    let data_url = format!("data:text/vtt;base64,{}", base64_content);

    Ok(SubtitleDownloadResult { vtt_data_url: data_url })
}
...
pub async fn search_subtitles_by_filename(
    ...
    let client = client.read().await;
    client.search_subtitles_by_filename(&filename, languages.as_deref()).await
}
