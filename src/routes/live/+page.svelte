<script lang="ts">
  ...
  function goBack() {
    if (history.length > 1) {
      ...
      goto('/?tab=channels', { replaceState: true });
    }
  }
  ...
  function switchChannel(direction: 1 | -1) {
    ...
    if (next) {
      goto(`/live?url=${encodeURIComponent(next.url)}&title=${encodeURIComponent(next.name)}&chIdx=${nextIdx}`, { replaceState: true });
    }
  }
  ...
  onMount(async () => {
    ...
    try {
      ...
      if (stored) channelList = JSON.parse(stored);
    } catch {}
    ...
    // Desktop: HTML5 video plays via the <video> element below
  });
