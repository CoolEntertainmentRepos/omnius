<script lang="ts">
  ...
  async function handlePlay() {
    ...
    try {
      // Start the stream
      const info = await startStream(selectedTorrent.hash);

      // Store stream info
      streamStore.streamInfo = info;
      streamStore.selectedTorrent = selectedTorrent;
      streamStore.isActive = true;

      // Navigate to fullscreen player with stream URL
      const params = new URLSearchParams({
        title: movie.title,
        imdb: movie.imdb_code || "",
        url: info.stream_url,
      });
      goto(`/player/${info.info_hash}?${params.toString()}`);
    } catch (err) {
      streamError = err instanceof Error ? err.message : "Failed to start stream";
    } finally {
