<script lang="ts">
  ...
  async function handlePlay() {
    ...
    try {
      ...
      await goto(`/movies/${movieId}/play/${selectedTorrent.hash}?${params.toString()}`);
    } catch (err) {
...
<div class="page">
  ...
  {:else if error}
    <div class="error-container">
      ...
      <div class="error-buttons">
        <button class="back-button" onclick={() => goto('/movies')}>
          <svg viewBox="0 0 24 24" fill="currentColor">
  ...
  {:else if movie}
    ...
    <!-- Back Button -->
    <nav class="nav">
      <button class="back-btn" onclick={() => goto('/movies')} aria-label="Go back">
        <svg viewBox="0 0 24 24" fill="currentColor">
          <path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z" />
        </svg>
      </button>
          ...
          {:else if movie.torrents && movie.torrents.length > 0}
            ...
            <div class="quality-section">
              <div class="quality-options">
                {#each movie.torrents as torrent (torrent.hash)}
                  <button
                    class="quality-btn"
                  ...
                  >
                    ...
                    <span class="quality-meta">{formatSize(torrent.size)}{torrent.seeds > 0 ? ` · ${torrent.seeds} seeds` : ''}</span>
                  </button>
                {/each}
              </div>
            </div>
